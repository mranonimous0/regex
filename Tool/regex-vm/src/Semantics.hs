...
interp_F p x =
  case p of {
   ExistT x0 p0 ->
    case x0 of {
     Empty -> Unknown;
     Eps -> Found (Result ([]) "" p0) __;
     Chr a' ->
      case p0 of {
       ([]) -> Unknown;
       (:) a s1 ->
        case ((Prelude.==) :: Prelude.Char -> Prelude.Char -> Prelude.Bool) a
               a' of {
         Prelude.True -> Found (Result ([]) ((:) a' "") s1) __;
         Prelude.False -> Unknown}};
     Cat e1 e2 ->
      case x (mk_input e1 p0) __ __ of {
       Unknown -> Unknown;
       Found r1 _ ->
     ...
     Choice e1 e2 ->
      case x (mk_input e1 p0) __ __ of {
       Unknown ->
        case x (mk_input e2 p0) __ __ of {
         Unknown -> Unknown;
